...
namespace OpenProtocolInterpreter.Tightening
{
    ...
    public class Mid0900 : Mid, ITightening, IController
    {
        ...
        private string ReReadField(int revision, DataFields field, ReadOnlySpan<char> package)
        {
            var dataField = GetField(revision, field);
            dataField.Value = GetValue(dataField, package).ToString();
        ...
        /// </summary>
        private static int ComputeVariableSectionSize(ReadOnlySpan<char> package, int sectionStartIndex, int count, int recordHeaderSize, int lengthOffset = 5)
        {
        ...
        private static int ReadInt(ReadOnlySpan<char> value, int index, int size)
        {
            if (index + size > value.Length)
                return 0;

            return OpenProtocolConvert.ToInt32(value.Slice(index, size).ToString());
        }
